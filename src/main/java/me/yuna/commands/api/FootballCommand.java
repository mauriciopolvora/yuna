...
public class FootballCommand extends Command {
	...
	public Match[] parseMatches(String responseBody) {
		...
		for (int i = 0; i < matchesJson.length(); i++) {
			...
			matches[i] = new Match(matchJson.getJSONObject("homeTeam").getString("name"),
					matchJson.getJSONObject("awayTeam").getString("name"), winner, matchJson.getString("utcDate"),
					matchJson.getInt("matchday"), matchJson.getJSONObject("season").getInt("currentMatchday"));
	...
	public static class Match {
		...
		public Match(String homeTeam, String awayTeam, String winner, String matchDate, int matchDay,
				int currentMatchDay) {
	...
	protected void execute(CommandEvent event) {
		...
		if (arguments[0].equalsIgnoreCase("equipas")) {
			...
			try {
				...
				event.reply("Cant fetch info from the given url.");
				e.printStackTrace();
			} catch (IOException e) {
				event.reply("An error has occured");
				e.printStackTrace();
			}
